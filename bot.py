...
# Auto generate cookies.txt from Environment Variable
YT_COOKIES_ENV = os.environ.get("YT_COOKIES", "").strip()
if YT_COOKIES_ENV:
    with open("cookies.txt", "w", encoding="utf-8") as f:
        f.write(YT_COOKIES_ENV)
    print("Cookies: Loaded from Environment Variable", flush=True)
...
def extract_and_download_social(url: str, user_id: int):
    ...
    # Automatically load cookies if available
...
async def main():
    ...
    async def start_cmd(client: Client, message: Message):
        ...
        text = (
            ...
            "Send any link to download media:\n"
        ...
        if social_match:
            ...
            try:
                ...
                if not file_path or not os.path.exists(file_path):
                    await status.edit_text("Failed to download video. Video might be private or unavailable.")
                    return
